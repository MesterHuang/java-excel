...
public class ExcelUtil<T> {
	...
	private HSSFRow row; // 行需要 判断确定（因此会共享）
	
	private boolean flag = true;

	private ExcelMergedRegion excelMergedRegion = new ExcelMergedRegion();
		...
		for (int i = 1; i < rows; i++) {
			...
			for (Field field : fields) {
				if(field.isAnnotationPresent(ExcelAttribute.class)){
					...
					if(excelMergedRegion.isMergedRegion(sheet, i, col)){
						// 以(行,列）作为一个元组
						String key = i+","+col;
						String value = excelMergedRegion.getMergedRegionValue(sheet, i, col);
